...
import { ReqPostHostDBInfo, ReqPostSiteDBInfo, ResGetHostDBInfo, ResGetSiteDBInfo } from '../../../lib/api/axios/types';

export type SiteSettingAddEditProps = {
  isNew: boolean;
  visible: boolean;
  close: () => void;
  data: ResGetSiteDBInfo[] | undefined;
  apply: (data: ReqPostSiteDBInfo) => void;
  applying: boolean;
};

export default function SiteSettingAddEdit({ isNew, visible, close, data, apply, applying }: SiteSettingAddEditProps) {
  return (
    <Drawer
      title={`${isNew ? 'New' : 'Edit'} Settings DataBase Info.`}
      placement="right"
